...
def command(args):
	...
	"""
	op = OptionParser(
		"%prog [--stdout] [-f settings] postgresql_bu.conf ([param=val]|[param])*",
		version = __version__
	)
	op.add_option(
		'-f', '--file',
		dest = 'settings',
		help = 'A file of settings to *apply* to the given "postgresql_bu.conf"',
		default = [],
		action = 'append',
	)
	op.add_option(
		'--stdout',
		dest = 'stdout',
		help = 'Redirect the product to standard output instead of writing back to the "postgresql_bu.conf" file',
		action = 'store_true',
		default = False
	)
