...
pg_home_directory = '.postgresql_bu'

# win32
pg_appdata_directory = 'postgresql_bu'
...
def defaults(environ = os.environ):
	...
	for k, v in (
		('sslcrtfile', os.path.join(pgdata, 'postgresql_bu.crt')),
		('sslkeyfile', os.path.join(pgdata, 'postgresql_bu.key')),
...
option_lib = optparse.make_option('-l',
	help = 'bind the library found in postgresql_bu.sys.libpath to the connection',
	type = 'str',
	dest = 'lib',
	action = 'append'
)
