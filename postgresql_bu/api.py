...
``postgresql_bu.api`` is a collection of Python APIs for the PostgreSQL DBMS. It
...
class Result(Element):
	...
	def parameters(self) -> (tuple, None):
		...
		`postgresql_bu.api.Database.cursor_from_id`.
		"""
...
class Transaction(Element):
	...
	def start(self) -> None:
		...
		applied to the transaction, raise a `postgresql_bu.exceptions.OperationError`.
		...
		`postgresql_bu.exceptions.OperationError`.
		"""
	begin = start
	...
	def __exit__(self, typ, obj, tb):
		...
		`postgresql_bu.exceptions.InFailedTransactionError`. If the database is
		unavailable, the `rollback` method should cause a
		`postgresql_bu.exceptions.ConnectionDoesNotExistError` exception to occur.
...
class SocketFactory(object):
	...
	def fatal_exception_message(self, err : Exception) -> (str, None):
		...
		given to the `postgresql_bu.exceptions.ConnectionFailure` instance that will
...
class Connector(Element):
	...
	def __init__(self,
		...
		if category is not None and not isinstance(category, Category):
			raise TypeError("'category' must a be `None` or `postgresql_bu.api.Category`")
...
class Installation(Element):
	...
	def version_info(self):
		...
		See `postgresql_bu.versionstring`.
		"""
...
class Cluster(Element):
	...
	def wait_until_started(self,
		...
		`postgresql_bu.exceptions.ClusterTimeoutError`.
		"""
	...
	def settings(self):
		"""
		A `Settings` interface to the ``postgresql_bu.conf`` file associated with the
		cluster.
		"""
