...
For more information on error codes see:
 http://www.postgresql_bu.org/docs/current/static/errcodes-appendix.html

This module is executable via -m: python -m postgresql_bu.exceptions.
It provides a convenient way to look up the exception object mapped to by the
given error code::

	$ python -m postgresql_bu.exceptions XX000
	postgresql_bu.exceptions.InternalError [XX000]
...
identity)::
	...
	$ python -m postgresql_bu.exceptions XX400
	postgresql_bu.exceptions.InternalError [XX000]

If that fails, it will return `postgresql_bu.exceptions.Error`
"""
import sys
import os
...
if __name__ == '__main__':
	for x in sys.argv[1:]:
		if x.startswith('01'):
			...
			e = ErrorLookup(x)
		sys.stdout.write('postgresql_bu.exceptions.%s [%s]%s%s' %(
