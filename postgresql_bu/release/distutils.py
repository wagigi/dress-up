...
should be of particular interest. If the distribution including ``py-postgresql_bu``
...
LONG_DESCRIPTION = """
py-postgresql_bu is a set of Python modules providing interfaces to various parts
of PostgreSQL. Notably, it provides a pure-Python driver + C optimizations for
querying a PostgreSQL database.

http://python.projects.postgresql_bu.org
...
Sample PG-API Code::
	...
	>>> import postgresql_bu
	>>> db = postgresql_bu.open('pq://user:password@host:port/database')
...
py-postgresql_bu is not yet another PostgreSQL driver, it's been in development for
years. py-postgresql_bu is the Python 3 port of the ``pg_proboscis`` driver and
...
def standard_setup_keywords(build_extensions = True, prefix = default_prefix):
	"""
	Used by the py-postgresql_bu distribution.
	"""
	d = {
		'name' : name,
		'version' : version,
...
class TestCommand(Command):
	...
	def run(self):
		import unittest
		unittest.main(module='postgresql_bu.test.testall', argv=('setup.py',))
